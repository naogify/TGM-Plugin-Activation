...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		/**
		 * Flag to show admin notices only for admin user.
		 *
		 * @since x.x.x
		 *
		 * @var boolean
		 */
		public $has_notices_admin = false;
		...
		public function notices() {
			...
			if ( ! empty( $message ) ) {
                ...
                var_dump($this);
                var_dump($this->has_notices_admin);
				if($this->has_notices_admin) {
					if ( current_user_can( 'install_plugins' ) ) {
						register_notice();
					}
				}else{
					...
					public function add_strings() {
						if ( 'update' === $this->options['install_type'] ) {
							...
							if ( version_compare( $this->tgmpa->wp_version, '4.8', '<' ) ) {
								$this->upgrader->strings['skin_update_successful'] .= ' <a href="#" class="hide-if-no-js" onclick="%2s"><span>' . esc_html__( 'Show Details', 'lightning' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'lightning' ) . '</span>.</a>';
							}
						}
					}
